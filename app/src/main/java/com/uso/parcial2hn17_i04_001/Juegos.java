...
public class Juegos extends AppCompatActivity {
    ...
    private SharedPreferences RespuestaNumero;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        RespuestaNumero = getSharedPreferences(ARCHIVOR, MODE_PRIVATE);
                ...
                if (!idJugador.getText().toString().equals("")) {
                    if (!validarEntrada()) {
                        ...
                        if (txtNumero.getText().toString().equals(aleatorio.getText().toString())) {
                            int s = 10;
                            puntos += s;
                            int  nuevospuntos = puntos + Integer.valueOf(idPuntos.getText().toString());
                            Toast.makeText(Juegos.this, "Respuesta Correcta!.", Toast.LENGTH_SHORT).show();
                            txtNumero.setText("");
                            idPuntos.setText(String.valueOf(nuevospuntos));
                            numeroAlazar();
                        } else {
                            int restapuntos = Integer.valueOf(idPuntos.getText().toString()) - 1;
                            idPuntos.setText(String.valueOf(restapuntos));
                            CargarNumero();
   ...
   private void GuardarNumero(){
       SharedPreferences.Editor editor = this.RespuestaNumero.edit();
       editor.putString(KEY_Respuesta, this.aleatorio.getText().toString());
       editor.commit();

   }
    private void CargarNumero(){
     if(this.RespuestaNumero !=null){
            this.aleatorio.setText(this.RespuestaNumero.getString(KEY_Respuesta,""));
        }
   }
